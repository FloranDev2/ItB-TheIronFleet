...
local function modApiExtGetFinalEffect(self, p1, p2, p3, parentSkill, ...)
	-- Dereference to weapon object
	if type(self) == "string" then
		self = _G[self]
	end
	                                                                         --type(parentSkill.GetSkillEffect) == "function"
	local isValidSkillTable = parentSkill and type(parentSkill) == "table" and type(parentSkill.GetFinalEffect) == "function" 
	local isPrimaryCall = not isValidSkillTable

	local skillFx = nil
	if isPrimaryCall then
		--skillFx = modApiExt_internal.oldSkills[self.__Id](self, p1, p2, getmetatable(self), ...)
		skillFx = modApiExt_internal.oldFinalSkills[self.__Id](self, p1, p2, p3, getmetatable(self), ...)
	else
		-- Defer to parent skill's GetSkillEffect.
		--skillFx = modApiExt_internal.oldSkills[parentSkill.__Id](self, p1, p2, getmetatable(parentSkill), ...)
		skillFx = modApiExt_internal.oldFinalSkills[parentSkill.__Id](self, p1, p2, p3, getmetatable(parentSkill), ...)
	end
	...
	if isPrimaryCall then
		...
		--I'm not sure if "fireSkillBuildSecondClickHooks" is supposed to be defined somewhere...
		modApiExt_internal.fireSkillBuildSecondClickHooks(
			modApiExt_internal.mission,
			Pawn, self.__Id, p1, p2, p3, skillFx
		)

		--Don't want to trigger skill start and skill end again -> EDIT: yes we do. They don't trigger for TC
		--But it might be useful to have that for two clicks skills in the future? -> YES
		if not skillFx.effect:empty() then
			local fx = SkillEffect()
			local effects = extract_table(skillFx.effect)
			...
			fx:AddScript(
				"modApiExt_internal.fireSkillTCStartHooks("
				.."modApiExt_internal.mission, Pawn,"
			...
			fx:AddScript(
				"modApiExt_internal.fireSkillTCEndHooks("
				.."modApiExt_internal.mission, Pawn,"
				.."\""..self.__Id.."\","..p1:GetString()..","..p2:GetString()..","..p3:GetString()..")"
			)

			--Shouldn't be useful, I guess?
			--[[
			if
				self == Prime_Punchmech    or
				self == Prime_Punchmech_A  or
		...
		--Queued effects, so vek attacks, right?
		--Vek don't have Two Clicks attacks, right??
		--So I can comment this, right???
		--[[
		if not skillFx.q_effect:empty() then
			local fx = SkillEffect()
			local effects = extract_table(skillFx.q_effect)
...
--TRUELCH
function modApiExtHooks:overrideFinalSkill(id, skill)
	assert(skill.GetFinalEffect)
	assert(_G[id] == skill) -- no fun allowed

	if modApiExt_internal.oldFinalSkills[id] then
		error(id .. " is already overridden!")
	end

	modApiExt_internal.oldFinalSkills[id] = skill.GetFinalEffect

	-- Make it possible to identify skills with no ambiguity
	skill.__Id = id
	skill.GetFinalEffect = modApiExtGetFinalEffect
end
...
function modApiExtHooks:overrideAllSkills()
	...
	--Two click
	if not modApiExt_internal.oldFinalSkills then
		modApiExt_internal.oldFinalSkills = {}

		for k, v in pairs(_G) do
			if type(v) == "table" and v.GetFinalEffect then
				self:overrideFinalSkill(k, v)
			end
		end
	end
end
